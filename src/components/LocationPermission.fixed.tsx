...
interface WeatherApiWeatherItem {
  description: string;
  icon: string;
  id: number;
  main: string;
...
interface WeatherApiResponse {
  ...
  weather: WeatherApiWeatherItem[];
  name: string;
}

// OpenStreetMap API response interface
...
const LocationPermission: React.FC = () => {
  ...
  const getMockAddressForCoordinates = (latitude: number, longitude: number): string => {
    ...
    // Aurangabad region
    else if (latitude >= 19.8 && latitude <= 19.9 && longitude >= 75.3 && longitude <= 75.4) {
      return "औरंगाबाद, महाराष्ट्र, भारत";
    } 
    // Nagpur region
      ...
      try {
        ...
        try {
          ...
          if (data.status === 'OK' && data.results && data.results.length > 0) {
            ...
            const localityResult = data.results.find((result: GoogleMapsResult) => 
            ...
            data.results.forEach((result: GoogleMapsResult) => {
  ...
  useEffect(() => {
    if ('geolocation' in navigator) {
      ...
      if (localStorage.getItem('locationPermission') === 'granted') {
        ...
        getCurrentLocation();
      } else if (localStorage.getItem('locationPermission') === 'denied') {
        setPermissionStatus('denied');
      } else {
