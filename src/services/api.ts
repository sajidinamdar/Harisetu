...
export const weatherAPI = {
  ...
  getForecastByCoords: async (lat: number, lon: number) => {
    return fetchAPI(`/weather/forecast/coordinates?lat=${lat}&lon=${lon}`);
  },

  getForecastByCity: async (city: string, state?: string, country: string = 'IN') => {
    return fetchAPI('/weather/forecast/city', {
      method: 'POST',
